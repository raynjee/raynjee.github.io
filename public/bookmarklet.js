/* RaynETS Bookmarklet — scrape translated chapters from any novel site.
 *
 * One-tap import: bookmark this script, then tap the bookmark on any novel
 * site to grab translated chapters and send them to RaynETS.
 *
 * Installation: see /#/bookmarklet for step-by-step instructions.
 */

(function () {
  if (window.__raynetsRunning) return;
  window.__raynetsRunning = true;

  /* ── Tiny UI ─────────────────────────────────────────────────── */
  const css = `
#raynets-bar{position:fixed;bottom:0;left:0;right:0;z-index:2147483647;
...
#raynets-bar .a-status{flex:1;min-width:0}
#raynets-bar .a-progress{height:3px;background:#333;margin-top:8px;border-radius:2px}
#raynets-bar .a-progress div{height:100%;background:#f5f5f5;border-radius:2px;transition:width .3s}
#raynets-bar button{background:#f5f5f5;color:#0a0a0a;border:none;
  padding:8px 18px;border-radius:6px;font:600 12px system-ui,sans-serif;
  cursor:pointer;white-space:nowrap}
#raynets-bar button:disabled{opacity:.35;cursor:default}
#raynets-bar button.a-cancel{background:transparent;color:#999;border:1px solid #444}
#raynets-close{position:absolute;top:8px;right:12px;
  background:none;border:none;color:#666;font-size:18px;cursor:pointer;padding:4px}

@media(max-width:600px){#raynets-bar{flex-direction:column;align-items:stretch;text-align:center}}
`;
  ...
  bar.id = "raynets-bar";
  bar.innerHTML = `
    <button id="raynets-close" title="Close">&times;</button>
    <div class="a-status">
      <span id="raynets-msg">Scanning for chapters…</span>
      <div class="a-progress"><div id="raynets-fill" style="width:0%"></div></div>
    </div>
    <button id="raynets-go" disabled>Send to RaynETS</button>
    <button id="raynets-cancel" class="a-cancel">Cancel</button>
  `;
  document.body.appendChild(bar);

  const msgEl = document.getElementById("raynets-msg")!;
  const fillEl = document.getElementById("raynets-fill")!;
  const goBtn = document.getElementById("raynets-go")! as HTMLButtonElement;

  document.getElementById("raynets-close")!.onclick = () => {
    bar.style.opacity = "0";
    setTimeout(() => { bar.remove(); style.remove(); }, 300);
    window.__raynetsRunning = false;
  };
  document.getElementById("raynets-cancel")!.onclick = () => {
    bar.remove();
    style.remove();
    window.__raynetsRunning = false;
  };
