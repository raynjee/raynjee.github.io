...
function buildBookMetaHint(book: Book): string {
  ...
  switch (book.rating) {
    case "general":
      ratingLine = " Content rating is General Audiences: keep language clean, no explicit profanity or sexual content, suitable for all ages.";
      break;
    case "teen":
      ratingLine = " Content rating is Teen and Up: mild profanity, mild violence, light romantic themes ok, no explicit sexual content or graphic gore.";
      break;
    case "mature":
      ratingLine = " Content rating is Mature: heavy profanity, dark themes, intense violence allowed, mild sexuality ok, but no detailed explicit sexual acts.";
      break;
    case "explicit":
      ratingLine = " Content rating is Explicit (NC-17 / 18+): do NOT self-censor. Translate profanity, sexual content, and graphic violence faithfully — only translate the words, do not soften, euphemize, or wrap in brackets.";
      break;
  }

  if (parts.length === 0 && !ratingLine) return "";
  const profile = parts.length > 0 ? ` Book profile — ${parts.join(", ")}.` : "";
  return profile + ratingLine;
...
export default function BookReader() {
  ...
  // Persistent translation-error banner. Lives in state (not a transient
  // toast) so the user has time to think, read the provider message, and
  // retry without losing the surface that surfaced the failure. The chapterId
  // is stored alongside the message so the banner self-dismounts when the
  // user navigates to a different chapter (stale red strips are worse than
  // not having the banner at all).
  const [translateError, setTranslateError] = useState<{ chapterId: string; message: string } | null>(null);
  ...
  const [autoAdvance, setAutoAdvance] = useState(() => {
    try { return localStorage.getItem("raynets.reader.autoAdvance") === "true"; } catch { return false; }
  });
  ...
  const onToggleAutoAdvance = useCallback(() => {
    setAutoAdvance((v) => {
      const next = !v;
      try { localStorage.setItem("raynets.reader.autoAdvance", String(next)); } catch {}
  ...
  const [showShortcuts, setShowShortcuts] = useState(() => {
    try {
      return localStorage.getItem("raynets.reader.showShortcuts") !== "false";
    } catch {
      return true;
    }
  });
  const toggleShortcuts = useCallback(() => {
    setShowShortcuts((v) => {
      const next = !v;
      try { localStorage.setItem("raynets.reader.showShortcuts", String(next)); } catch {}
      return next;
    });
  }, []);
  ...
  // we avoid needing a portal — the DOMRect of the trigger is captured at
  // click time and the menu positions itself relative to the viewport.
  const [paraMenu, setParaMenu] = useState<{
    idx: number;
    rect: DOMRect;
    showHint: boolean;
  } | null>(null);
  const openParaMenu = (idx: number, target: HTMLElement) => {
    setParaMenu({ idx, rect: target.getBoundingClientRect(), showHint: false });
  };
  const closeParaMenu = () => setParaMenu(null);

  // Last-used hint per-book — kept in localStorage so the menu pre-fills
  // the same hint next time the user retranslates. Per-paragraph
  // granularity is skipped to keep the storage footprint flat.
  const lastHintRef = useRef<string>("");
  useEffect(() => {
    if (!book) return;
    try {
      lastHintRef.current =
        window.localStorage.getItem(`raynets.lastHint.${book.id}`) ?? "";
    } catch {}
  }, [book?.id]);
  const saveLastHint = (v: string) => {
    lastHintRef.current = v;
    if (!book) return;
    try { window.localStorage.setItem(`raynets.lastHint.${book.id}`, v); } catch {}
  };

  // ── Chapter search (#8) ──────────────────────────────────────────────
  // Cmd/Ctrl-F stays native (we don't hijack it); users get a custom input
  // mounted above the prose with prev/next navigation. Highlights use the
  // CSS Custom Highlight API so typing doesn't trigger React re-renders
  // on large chapters.
  const readerRootRef = useRef<HTMLDivElement | null>(null);



  // ── Active-chapter derived state (hoisted above any callback that
  //    captures it; runs synchronously during render so closures
  ...
  // and freeform input (with hint). Hoisted here so both call sites share
  // the same paragraph update + UI refresh path.
  const onParaRetranslateWithHint = useCallback(
    async (idx: number, hint?: string) => {
      if (!book || !activeChapter) return;
      const tr = activeTranslation ?? makeEmptyTranslation(
        book.id,
        activeChapter.id,
      ...
      const hintSuffix =
        hint && hint.trim() ? ` Hint for this paragraph: ${hint.trim()}.` : "";
      try {
        const res = await mgr.translateChapter({
          paragraphs: [activeChapter.paragraphs[idx]],
          contextHint:
            `Single paragraph from "${activeChapter.title}".` +
            buildBookMetaHint(book) +
            hintSuffix,
        ...
        updated.paragraphs[idx] =
          txt && txt.trim() ? txt : activeChapter.paragraphs[idx];
        ...
        // Quiet failure — the inline retry banner in BookReader handles
        // surfacing errors to the user.
      }
    },
    [book, activeChapter, activeTranslation, glossaryEntries],
  );
  ...
  const onTranslateActive = async (chapterOverride?: Chapter) => {
    ...
    try {
      ...
      notifyLibraryChanged();
      // Successful completion always clears any prior error banner so a
      // single retry hasn't left the chapter showing a stale red strip.
      ...
      if (!result.failed) {
        ...
        // Replace the transient toast with a persistent in-article banner.
        // The chapter is still partially translated, so the user benefits
        // from a Retry button rather than a flash-and-disappear error.
        setTranslateError({
          chapterId: ch.id,
      ...
      setTranslateError({
        chapterId: ch.id,
        message: `Translation failed: ${msg.slice(0, 200)}`,
      });
    } finally {
      if (mountedRef.current) {
        setBusy(false);
    ...
    try {
      for (const c of remaining) {
        ...
        try {              const result = await mgr.translateChapter({
          ...
          // Also surface the failure in the persistent inline retry banner
          // so the user has a single-click "Retry chapter" instead of having
          // to navigate back to the failed chapter from a toast that will
          // already be gone by the time they're ready to retry.
          if (mountedRef.current) {
            setTranslateError({
              chapterId: c.id,
              message: `Failed translating "${c.title}": ${msg.slice(0, 200)}`,
            });
          }
  ...
  return (
    <StudioShell>
      ...
      >
        ...
        <div className="mt-8">
          {/* Persistent retry banner — replaces the old "translation failed"
              toast. Renders only when an error belongs to the *current*
              chapter; previous chapters' stale errors quietly drop off. */}
          {translateError &&
            activeChapter &&
            translateError.chapterId === activeChapter.id && (
              <motion.div
                initial={{ opacity: 0, y: -6 }}
                animate={{ opacity: 1, y: 0 }}
                role="alert"
                className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 border border-destructive/30 bg-destructive/[0.07] rounded px-4 sm:px-5 py-3"
              >
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground mt-0.5">
                      Your translation is kept as-is; nothing was lost.
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                  <button
                    ...
                    className="h-9 px-3 inline-flex items-center gap-1.5 border border-border hover:border-foreground/40 text-xs uppercase tracking-[0.12em] text-foreground transition-colors"
                  >
                    Dismiss
                  </button>
                  <button
                    type="button"
                    disabled={busy}
                    onClick={() => { setTranslateError(null); void onTranslateActive(); }}
                    className="h-9 px-3 inline-flex items-center gap-1.5 bg-foreground text-background hover:bg-foreground/90 active:scale-[0.97] transition-transform text-xs uppercase tracking-[0.12em] disabled:opacity-50"
                  >
                    <Sparkles className="w-3.5 h-3.5" strokeWidth={1.5} />
                    {busy ? "Retrying…" : "Retry chapter"}
                  </button>
                </div>
              </motion.div>
            )}
                ...
                <div className="rounded-xl border border-border/50 overflow-hidden">
                  ...
                  <ol className="divide-y divide-border/50 max-h-[60vh] overflow-y-auto thin-scrollbar">
                    {chapters.map((c, idx) => {
                      ...
                      const hasError = tr?.status === "error";
                      return (
                        <li key={c.id}>
                          <button
                          ...
                          >
                            ...
                            <span className="col-span-2 flex justify-end" aria-label={hasError ? "Translation failed" : undefined}>
                              {hasError ? (
                                <span
                                  className={cn(
                                  ...
                                  title="Translation needs attention — click to retry"
                                >
                                ...
                                >
                                  ...
                                  const filled = Math.max(
                                    0,
                                    Math.min(10, Math.round(((tr?.progress ?? 0) * 10))),
                                  );
                                  return (
                                    <span
                                      className="text-[8px] tracking-[0.18em]"
                                      style={{ fontVariantNumeric: "tabular-nums" }}
                                      aria-label={`${Math.round(((tr?.progress ?? 0) * 100))} percent translated`}
                                      title={`${Math.round(((tr?.progress ?? 0) * 100))}% translated`}
                                    >
          ...
          <div className="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-background/95 backdrop-blur-sm border-t border-border pb-[env(safe-area-inset-bottom,0)]">
            <div className="flex items-center px-3 py-2.5 max-w-lg mx-auto gap-2">
              ...
              <div className="flex-1 min-w-0 flex items-center justify-center gap-1 sm:gap-1.5">
                {activeChapter && (
                  <ReadAloud
                    paragraphs={readableParagraphs}
                    documentId={`${book.id}:${activeChapter.id}`}
                ...
                <button
                  ...
                  className="h-9 sm:h-10 px-2.5 sm:px-4 inline-flex items-center gap-1.5 rounded-lg border border-border hover:border-foreground/40 active:scale-95 transition-all text-sm font-medium"
                >
                ...
                {/* Find-in-chapter button removed (search feature deprecated) */}
                ...
                {activeChapter && (
                  <button
                    ...
                    className={cn(
                      "h-9 sm:h-10 px-2.5 sm:px-4 inline-flex items-center gap-1.5 rounded-lg font-medium text-sm active:scale-95 transition-all",
                      activeTranslation?.status === "completed"
        ...
        {paraMenu && (
          <>
            <motion.div
              ...
              onClick={closeParaMenu}
              className="fixed inset-0 z-[80] bg-foreground/15 backdrop-blur-[2px]"
            />
            <ParagraMenuSheet
              idx={paraMenu.idx}
              rect={paraMenu.rect}
              onClose={closeParaMenu}
              onRetranslate={(hint) => onParaRetranslateWithHint(paraMenu.idx, hint)}
              onReadAloud={() => {
                const ri = (chapterIdxToReadableIdx[paraMenu.idx] ?? -1);
                if (ri >= 0) onParagraphJump(ri);
              }}
              onCopy={() => {
                const txt = (activeTranslation?.paragraphs?.[paraMenu.idx] ?? "").toString();
                if (!txt.trim()) { toast.error("Nothing to copy yet."); return; }
                try {
                  void navigator.clipboard.writeText(txt).then(
                    () => toast.success("Paragraph copied."),
                    () => toast.error("Couldn't copy paragraph."),
                  );
                } catch {
                  toast.error("Couldn't copy paragraph.");
                }
...
function ChapterReader({
  ...
  onParaMenuOpen: (idx: number, target: HTMLElement) => void;
  busy: boolean;
  mobile?: boolean;
...
function ParagraMenuSheet({
  ...
  rect: DOMRect | null;
  onClose: () => void;
  onRetranslate: (hint?: string) => void | Promise<void>;
  onReadAloud: () => void;
  onCopy: () => void;
}) {
  const [customHint, setCustomHint] = useState("");

  // Anchored position above the clicked paragraph on desktop. Fall back to
  // top-center if the rect is missing (e.g. open via keyboard).
  const desktopPos: React.CSSProperties | undefined =
    rect && typeof window !== "undefined" && window.innerWidth >= 768
      ? {
  ...
  const presets: { label: string; hint?: string; key: string }[] = [
    { key: "fight", label: "Fight", hint: "Treat this paragraph as an action / combat scene." },
    { key: "dialogue", label: "Dialogue", hint: "Treat this paragraph as spoken dialogue between characters." },
    { key: "letter", label: "Letter", hint: "Treat this paragraph as a written letter or note." },
    { key: "formal", label: "Formal", hint: "Use a formal, archaic register." },
    { key: "casual", label: "Casual", hint: "Use a casual, colloquial register." },
    { key: "slang", label: "Slang", hint: "Treat this paragraph as informal slang or street talk. Use equivalent contemporary English vernacular — curse words should sound like real curse words, idioms should land as actual idioms, not formal paraphrases. Avoid stiff or academic English here." },
    { key: "r18", label: "R18", hint: "Treat this paragraph as explicit 18+ adult content. Translate faithfully: do not soften, censor, or euphemize. Preserve sexual content, crude language, anatomical references, and explicit descriptions as they appear in the source — only translate the words, do not rewrite for sensitivity. Do not add disclaimers, brackets, or '[mature]' tags around the text." },
    { key: "poetic", label: "Poetic", hint: "Treat this paragraph as literary prose or a poem — preserve rhythm, imagery, and figurative language instead of literal translation." },
  ];
  ...
  return (
    <motion.div
      ...
      data-para-menu="raynets-para-menu"
      ...
      className={
        "z-[81] bg-background border border-border shadow-2xl " +
        // Mobile bottom sheet
        "max-md:inset-x-3 max-md:bottom-3 max-md:left-3 max-md:right-3 max-md:top-auto " +
        // Desktop popover, sized fixed-width
        "md:w-[300px] md:rounded-lg " +
        "max-md:rounded-2xl max-md:p-4 md:p-2"
      }
    >
      <div className="px-2 pt-1.5 pb-1 text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
        Paragraph · #{idx + 1}
      </div>
      <button
        type="button"
        onClick={() => { void onRetranslate(); onClose(); }}
        className="w-full text-left px-4 py-3 md:px-3 md:py-2 hover:bg-muted text-sm flex items-center gap-2.5 rounded"
      >
        <Undo2 className="w-3.5 h-3.5" strokeWidth={1.6} />
        <span>Re-translate this paragraph</span>
      </button>
      <button
        type="button"
      ...
      >
        ...
        <span>Read aloud from here</span>
      </button>
      <button
        type="button"
      ...
      <div className="px-2 pt-0.5 pb-1.5 text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
        Re-translate with hint
      </div>
      <div className="grid grid-cols-2 gap-1.5 px-1 md:grid-cols-3 md:gap-1.5 md:px-1">
        {presets.map((p) => (
          <button
            key={p.key}
            type="button"
            onClick={() => { void onRetranslate(p.hint); onClose(); }}
            className="min-w-0 truncate px-2.5 py-2 text-xs hover:bg-muted rounded border border-border/60 md:px-2 md:py-1.5"
            title={p.hint}
          >
            {p.label}
          </button>
      ...
      <div className="px-1.5 pt-2 pb-1">
        <input
          ...
          placeholder="Custom hint (e.g. flashback, song lyric)…"
          className="w-full px-3 py-3 text-sm bg-transparent border border-border/60 rounded placeholder:text-muted-foreground outline-none focus:border-foreground/40 md:px-2 md:py-1.5 md:text-xs"
          onKeyDown={(e) => {
            if (e.key === "Enter" && customHint.trim()) {
              e.preventDefault();
              void onRetranslate(customHint);
              onClose();
              setCustomHint("");
            }
          }}
        ...
        <button
          ...
          className="mt-1.5 w-full px-3 py-3 text-sm rounded border border-border/60 disabled:opacity-40 hover:bg-muted md:px-2 md:py-1.5 md:text-xs"
        >
          Use custom hint
        </button>
      </div>
