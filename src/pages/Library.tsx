...
import type { Chapter, ContentRating} from "@/lib/types";
  ...
  return (
    <StudioShell>
      <div className="mx-auto max-w-[1400px] px-6 lg:px-10 pt-12 pb-24">
        ...
        {/* ── Continue Reading ────────────────────────────────── */}
        {bookmark && (() => {
          // Resolve the chapter index before paint so the card reads as
          // "Ch 23 / 67" without needing a DB lookup. If the bookmark
          // somehow points at a chapter no longer in chapterOrder, we
          // degrade gracefully to "first chapter".
          const idx = Math.max(
            0,
            bookmark.book.chapterOrder.indexOf(bookmark.chapterId),
          );
          const total = bookmark.book.chapterOrder.length;
          return (
            <motion.div
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground mt-0.5">
                  {bookmark.book.author} · Ch {idx + 1} / {total} · Saved{" "}
              ...
              <button
                type="button"
                onClick={() =>
                  navigate(`/library/${bookmark.book.id}/${bookmark.chapterId}`)
                }
  ...
  return (
    <div className="gallery-grid">
      {Array.from({ length: 8 }).map((_, i) => (
        // Soul of the loading state: skeleton-cover blocks with no shimmer
        // gradient, no glossy animation. The slow `opacity` pulse lives on
        // the `.skeleton` class so theme tokens adapt when the user later
        // switches to a warm or sepia theme.
        <div key={i} className="flex flex-col h-full">
          <div className="skeleton aspect-[3/4] rounded-md" />
          <div className="mt-2.5 px-0.5">
            <span className="skeleton-line w-2/3" />
            <span className="skeleton-line w-4/5 mt-1.5" />
            <span className="skeleton-line w-1/2 mt-1" />
          </div>
        </div>
      ))}
    </div>
...
export function BookEditor({ bookId }: { bookId: string }) {
  ...
  const [rating, setRating] = useState<ContentRating>((book?.rating as ContentRating | undefined) ?? "");
  ...
  const origRatingRef = useRef<ContentRating>((book?.rating as ContentRating | undefined) ?? "");
  ...
  useEffect(() => {
    ...
    origRatingRef.current = (book?.rating as ContentRating | undefined) ?? "";
  }, [book?.id]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 mt-8">
          ...
          <section className="col-span-1 lg:col-span-8">
            <div className="space-y-4">
              ...
              {/* Editorial details — who the translator is writing for */}
              <div className="studio-card p-4 sm:p-5">
                <div className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground mb-3">Editorial details</div>
                <p className="text-[11px] text-muted-foreground mb-4 leading-relaxed">
                  Who is this book written for? The translator uses this to calibrate censoring and explicitness defaults across every translation call.
                </p>
{/* AO3-style rating selector (G / T / M / E) — drives censoring intensity across every translation call. */}
                <div className="mb-4">
                  <div className="flex items-center justify-between gap-2 mb-1.5">
                    <span className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Content rating</span>
                    {rating && (
                      <button
                        type="button"
                        onClick={() => setRating("")}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                    {[
                      { v: "general", l: "General Audiences", d: "Clean for all ages." },
                      { v: "teen", l: "Teen & Up", d: "Mild profanity + themes." },
                      { v: "mature", l: "Mature", d: "Heavy themes allowed." },
                      { v: "explicit", l: "Explicit", d: "No self-censoring." },
                    ].map((r) => (
                      <button
                        key={r.v}
                        type="button"
                        onClick={() => setRating(r.v === rating ? "" : (r.v as ContentRating))}
                        title={r.d}
                        aria-pressed={rating === r.v}
                        className={
                          "min-w-0 truncate px-2 py-2 text-xs rounded border transition-colors " +
                          (rating === r.v
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1.5 leading-relaxed">
                    Sets the censoring / explicitness defaults for the whole book. You can still tap the R18 chip on individual paragraphs to override.
                  </p>
                </div>
              </div>

              {/* Book metadata — genre, tone, style, audience, rating */}
              <div className="studio-card p-4 sm:p-5">
