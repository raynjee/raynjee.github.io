...
const SETTINGS_KEY = "raynets.settings.v1";
...
export function db(): Promise<IDBDatabase> {
  ...
  const globalW = window as unknown as { __raynetsDb?: IDBDatabase };
  if (globalW.__raynetsDb) return Promise.resolve(globalW.__raynetsDb);
  return openDb().then((d) => {
    globalW.__raynetsDb = d;
    d.onclose = () => {
      globalW.__raynetsDb = undefined;
    };
    return d;
  });
}
...
export function loadProviderStatus(): ProviderStatus[] {
  if (typeof window === "undefined") return [];
  try {
    const raw = window.localStorage.getItem("raynets.provider-status.v1");
