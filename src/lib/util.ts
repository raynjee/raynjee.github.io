...
// Single WPM assumption across all languages is intentional: precise
// per-language WPM math buys us nothing on the library tile (the user
// only needs a feel for "how much is left"), and 250 wpm is a widely
// accepted fiction-reading baseline.
const READING_WPM = 250;

export function readingTimeMinutes(totalWords: number): number {
  if (!Number.isFinite(totalWords) || totalWords <= 0) return 0;
  return Math.max(1, Math.ceil(totalWords / READING_WPM));
}

// "1h 12m", "42m", "<1m" — designed to slot into a tight tile stat row
// without breaking a 320px-wide layout. A 0-word book returns "<1m".
export function formatReadingTime(totalWords: number): string {
  const mins = readingTimeMinutes(totalWords);
  if (mins < 1) return "<1m";
...
const BOOKMARK_KEY = "raynets.reader.bookmark";
