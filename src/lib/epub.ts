...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  ...
  // coverDataUrl is a `data:<mime>;base64,<payload>` string set from the
  // Library cover uploader or extracted from the original EPUB manifest.
  // Parse defensively, skip silently if missing or unsupported so a
  // text-only book still exports cleanly without an empty <img>.
  const cover = parseCoverDataUrl(opts.book.coverDataUrl);
  let coverMeta = "";
  let coverManifest = "";
  let coverSpine = "";
  if (cover) {
    const imgPath = `images/cover.${cover.ext}`;
    // Write the binary image. JSZip's `{ base64: true }` flag lets us pass
    // the raw base64 string and have it decoded on write — avoids pulling
    // a 4 MB buffer through `atob` ourselves.
    zip.file(`OEBPS/${imgPath}`, cover.base64, { base64: true });
    // Wrap the image in a tiny XHTML so it actually RENDERS when the book
    // opens. EPUB3 technically only needs the manifest `properties="cover-image"`
    // for metadata, but readers (Calibre, Kobo, iBooks) consistently need
    // the spine item to actually display the cover on the first page.
    const coverHtml = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops">
<head><meta charset="utf-8"/><title>Cover</title>
<style>body { margin: 0; padding: 0; text-align: center; background: #fff; } img { max-width: 100%; height: auto; }</style>
</head>
<body>
<img src="${imgPath}" alt="Cover"/>
    ...
    coverManifest = `<item id="cover-img" href="${imgPath}" media-type="${cover.mime}" properties="cover-image"/>\n<item id="cover-page" href="cover.xhtml" media-type="application/xhtml+xml"/>`;
...
// ── Cover-data-url parser ─────────────────────────────────────────────────
// Returns `{ mime, ext, base64 }` for a data URL we can write into the zip,
// or `null` if it is missing, malformed, or uses an unsupported image type.
// Whitelist matters: SVG and BMP in particular cause epub-validation failures
// on Amazon KDP and on some older Kobo and Aldiko builds, so we silently
// skip them rather than ship a broken book.
...
function parseCoverDataUrl(url: string | null): ParsedCover | null {
  if (!url || typeof url !== "string") return null;
  // Accept the canonical `data:<mime>;base64,<payload>` shape with optional
  // whitespace around the delimiters (some browsers / OS image pickers
  // produce formatted-but-still-valid base64 with stray line breaks).
  const match = url.match(
    /^\s*data:(image\/(?:jpeg|jpg|png|webp|gif))\s*;\s*base64\s*,\s*([\s\S]+)\s*$/i,
  );
  if (!match) {
    if (url.startsWith("data:")) {
      console.warn(
        "Cover skipped: only PNG, JPEG, WebP, GIF data URLs are supported.",
      );
    }
    return null;
  }
  // Normalise `image/jpg` → `image/jpeg` and use a matching file extension
  // so the manifest media-type matches the on-disk filename.
